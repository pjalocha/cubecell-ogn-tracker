...
static uint32_t CRC30_Syndrome[SignBits];
static uint64_t CRC30_SyndromeSorted[SignBits];

static void SyndromeSort(void)
{ for(int Bit=0; Bit<SignBits; Bit++)
  { uint64_t Syndr = CRC30_Syndrome[Bit]; Syndr = (Syndr<<32) | Bit;
    CRC30_SyndromeSorted[Bit] = Syndr; }
  std::sort(CRC30_SyndromeSorted, CRC30_SyndromeSorted+SignBits); }

uint16_t CRC30_FindSyndrome(uint32_t Syndr)
{ uint16_t Bot=0;
  uint16_t Top=SignBits;
  uint32_t MidSyndr=0;
  for( ; ; )
  { uint16_t Mid=(Bot+Top)>>1;
    MidSyndr = CRC30_SyndromeSorted[Mid]>>32;
    if(Syndr==MidSyndr) return (uint16_t)CRC30_SyndromeSorted[Mid];
    if(Mid==Bot) break;
...
{ if(Syndr==0) return 0;
  ...
  { uint16_t Bit1 = (uint16_t)BitWeiSorted[Idx];
    ...
    if(Wei1>=512) break;
    uint16_t Bit2 = CRC30_FindSyndrome(Syndr^CRC30_Syndrome[Bit1]);
    if(Bit2<SignBits)
    { FlipBit(Sign, Bit1); Syndr^=CRC30_Syndrome[Bit1];
      FlipBit(Sign, Bit2); Syndr^=CRC30_Syndrome[Bit2];
      return 2; }
  }
  ...
  { uint16_t Bit1 = (uint16_t)BitWeiSorted[Idx1];
    ...
    { uint16_t Bit2 = (uint16_t)BitWeiSorted[Idx2];
      ...
      uint16_t Bit3 = CRC30_FindSyndrome(Syndr^CRC30_Syndrome[Bit1]^CRC30_Syndrome[Bit2]);
      if(Bit3<SignBits)
      { FlipBit(Sign, Bit1); Syndr^=CRC30_Syndrome[Bit1];
        FlipBit(Sign, Bit2); Syndr^=CRC30_Syndrome[Bit2];
        FlipBit(Sign, Bit3); Syndr^=CRC30_Syndrome[Bit3];
        return 3; }
    }
  }
  ...
  { uint16_t Bit1 = (uint16_t)BitWeiSorted[Idx1];
    ...
    { uint16_t Bit2 = (uint16_t)BitWeiSorted[Idx2];
      ...
      { uint16_t Bit3 = (uint16_t)BitWeiSorted[Idx3];
        ...
        uint16_t Bit4 = CRC30_FindSyndrome(Syndr^CRC30_Syndrome[Bit1]^CRC30_Syndrome[Bit2]^CRC30_Syndrome[Bit3]);
        if(Bit4<SignBits)
        { FlipBit(Sign, Bit1); Syndr^=CRC30_Syndrome[Bit1];
          FlipBit(Sign, Bit2); Syndr^=CRC30_Syndrome[Bit2];
          FlipBit(Sign, Bit3); Syndr^=CRC30_Syndrome[Bit3];
          FlipBit(Sign, Bit4); Syndr^=CRC30_Syndrome[Bit4];
          return 3; }
      }
    }
  }
...
{ Count_Trials++;
  ...
  if(Syndr!=0)
  { uint16_t BadBit = CRC30_FindSyndrome(Syndr);
    // printf("Syndr:%08X => [%04X]\n", Syndr, BadBit);
    if(BadBit<SignBits)
    { FlipBit(DecodedSign, BadBit);
      Syndr^=CRC30_Syndrome[BadBit];
      Count_FixedCRC++; }
  }
  if(Syndr!=0)
...
{ time_t Now; time(&Now); srand(Now);
  ...
  { FlipBit(Sign, Bit);
    uint32_t Check = CheckSignCRC(Sign);
    // printf("[%4d] = %08X\n", Bit, Check);
    CRC30_Syndrome[Bit]=Check;                  // store single bit error syndrome in a tabel
    FlipBit(Sign, Bit); }
  SyndromeSort();                               // produce sorted table for a quick search

  // test if syndrome search es correctly
  for(int Bit=0; Bit<SignBits; Bit++)
  { int ErrBit=CRC30_FindSyndrome(CRC30_Syndrome[Bit]);
    if(ErrBit!=Bit) printf("CRC30_FindSyndrome() failed %d<=>%d\n", Bit, ErrBit); }
