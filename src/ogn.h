...
{ public:
  ...
  public:
   ...
   { if(Packet.Header.Emergency) { Rank=0xFF; return; }                 // emergency packets always highest rank
     Rank=1;                                                            // always give at least one point
     ...
     if(RxRSSI>128)                                                     // [-0.5dB] weaker signal => higher rank
       Rank += (RxRSSI-120)>>1;                                         // 1 point/1dB below -60dBm
     ...
     if(RxAltitude>0)                                                   //
       Rank += RxAltitude>>5;                                           // 1 points/32m of altitude below
     int16_t ClimbRate = Packet.DecodeClimbRate();                      // [0.1m/s] higher sink rate => higher rank
     if(ClimbRate<0)
       Rank += (-ClimbRate)>>2;                                         // 1 point/0.4m/s of sink
   }
